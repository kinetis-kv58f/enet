...
void uart0_puts(uint16_t const len, char const * const data) {
...
static inline void priv_setup_pins(void) {
	...
	hw_portB->PCR[rxPinNum] = HwPort_MuxAlt7 << HwPort_PCR_MUX_Shift;
	hw_portB->PCR[txPinNum] = HwPort_MuxAlt7 << HwPort_PCR_MUX_Shift;
}
