...
/// VDIV = 38
/// PRDIV = 4
/// PLL = (OSCCLK * VDIV)/(PRDIV * 2) = 237500000
///
/// MCG_MCGOUTCLK:
/// MCGOUTCLK = PLL
///
/// SIM:
/// CoreCLK = MCGOUTCLK/OUTDIV1 = MCGOUTCLK/1 = 237500000
/// FastPhCLK = MCGOUTCLK/OUTDIV2 = MCGOUTCLK/2 = 118750000
/// FlexbusCLK = MCGOUTCLK/OUTDIV3 = MCGOUTCLK/4 = 59375000
/// BusFlashCLK = MCGOUTCLK/OUTDIV4 = MCGOUTCLK/8 = 29687500
///
...
static inline void priv_init_sim(void) {
	...
	hw_sim->CLKDIV1 |= (uint32_t) 0x3 << SIM_CLKDIV1_OUTDIV3_Shift;

	// OUTDIV4: /8;
	hw_sim->CLKDIV1 &= ~SIM_CLKDIV1_OUTDIV4_Mask;
	hw_sim->CLKDIV1 |= (uint32_t) 0x07 << SIM_CLKDIV1_OUTDIV4_Shift;
...
static void priv_setup_clocks(void) {
	...
	// Enable HSRUN mode
	{
		hw_smc->PMPROT |= SMC_PMPPROT_AHSRUN_Mask;
		hw_smc->PMCTRL = (hw_smc->PMCTRL & ~SMC_PMCTRL_RUNM_Mask)
				| SMC_PMCTRL_RUNM_Hsrun;
		while (hw_smc->PMSTAT != SMC_PMSTAT_Hsrun)
			;
	}
...
void cpu_trigger_pendsv(void) {
	hw_scb->ICSR |= Scb_ICSR_PENDSVSET_Mask;
}
