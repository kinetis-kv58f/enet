...
/// \param byte Input byte
/// \returns Higher nibble
static inline uint8_t uint8_make_from_nibbles(uint8_t const hiNib,
		uint8_t const loNib);
static inline uint8_t uint8_make_from_nibbles(uint8_t const hiNib,
		uint8_t const loNib) {
...
static inline uint16_t uint16_swap_bytes(uint16_t const bytes) {
	return (uint16_t) (((bytes >> 8U) & 0xFFU)
			| (uint16_t) ((bytes & 0xFFU) << 8U));
}
...
/// \param lo Lower byte
/// \returns 16-bit data
static inline uint16_t uint16_make_from_bytes(uint8_t const hi,
		uint8_t const lo);
static inline uint16_t uint16_make_from_bytes(uint8_t const hi,
		uint8_t const lo) {
	uint16_t ret = hi;
...
/// \returns 16-bit data
static inline uint32_t uint32_make_from_uint16(uint16_t const hi,
		uint16_t const lo);
static inline uint32_t uint32_make_from_uint16(uint16_t const hi,
		uint16_t const lo) {
	uint32_t ret = hi;
...
static inline uint32_t uint32_make_from_uint8(uint8_t const b3,
		uint8_t const b2, uint8_t const b1, uint8_t const b0);
static inline uint32_t uint32_make_from_uint8(uint8_t const b3,
		uint8_t const b2, uint8_t const b1, uint8_t const b0) {
	uint32_t ret = b3;
	ret <<= 8U;
...
/// \param hw3 Halfword3
/// \returns 64-bit data
static inline uint64_t uint64_make_from_uint16(uint16_t const hw0,
		uint16_t const hw1, uint16_t hw2, uint16_t hw3);
static inline uint64_t uint64_make_from_uint16(uint16_t const hw0,
		uint16_t const hw1, uint16_t hw2, uint16_t hw3) {
	uint64_t ret = hw3;
	ret <<= 16U;
...
/// \param w1 Word1
/// \returns 64-bit data
static inline uint64_t uint64_make_from_uint32(uint32_t const w0,
		uint32_t const w1);
static inline uint64_t uint64_make_from_uint32(uint32_t const w0,
		uint32_t const w1) {
	uint64_t ret = w1;
...
/// \param inp2 Input2
/// \returns Absolute subtraction of two bytes
static inline uint16_t uint8_abs_subtract(uint8_t const inp1,
		uint8_t const inp2) {
	if (inp2 >= inp1) {
		return inp2 - inp1;
	} else {
...
#if 0
static inline uint16_t uint16_abs(int16_t const in) {
	int16_t const mask = (in) >> ((sizeof(uint16_t) * 8) - 1);

	return (in + mask) ^ mask;
}
#endif

/// Returns time difference between two time stamps
/// \param earliest First time stamp
/// \param latest Second time stamp
/// \returns Time difference between two time stamps
static inline uint16_t uint16_time_difference(uint16_t const earliest,
		uint16_t const latest) {
	if (latest >= earliest) {
		return latest - earliest;
	} else {
		return latest + (UINT16_MAX - earliest);
	}
}
